    ...
    struct ghc_codec* encoder, uint8_t* zeros, uint16_t pos_plod)
{
    ...
    *zeros = 0;
    ...
    for (i = 0; i < max_len; ++i) {
        if(0 != encoder->decoded[pos_plod + i]) {
            break;
        }
        else {
            ++*zeros;
    ...
    uint16_t* match_len, uint16_t max_len, uint16_t pos_dict, uint16_t pos_plod)
{
    ...
    *match_len = 0;
    for (i = 0; i < max_len; ++i) {
        if(encoder->decoded[pos_dict + i] !=
           encoder->decoded[pos_plod + i]) {
               break;
        }
        else {
            ++*match_len;
    ...
    *max_len = 0;
    ...
    for( dict_pivot = 0; dict_pivot <= (pload_idx - GHC_BREF_CNT_VALUE_MIN); ++dict_pivot) {
        ...
        printf("%s:%d: pload_idx: %03d dict_pivot: %03d "
            ...
            match_len, *max_len, encoder->decoded[pload_idx]);
#endif
    ...
    uint16_t longest_match_len, uint16_t longest_match_offs)
{
    ...
    while ((match_len & ~EXT_FULL_LEN_MASK) ||
        ...
        encoder->encoded[encoder->pos_enco] = bref_bc;
        ++encoder->pos_enco;
    }
...
/*
 * encoder.pos_deco points at the first decoded entry
 * encoder.pos_enco points at the first decoded entry
 */
int ghc_encode(struct ghc_codec* encoder)
{
    ...
    while (clean && comp_not_full(encoder) && payload_not_empty(encoder))
    {
        ...
        if (1U < zero_seq_len || 1U < longest_match_len ||
            ...
            if (copy_seq_len) {
                encoder->encoded[encoder->pos_enco] =
                    GHC_COPY_BC | (copy_seq_len & GHC_COPY_CNT_MASK);
                ++(encoder->pos_enco);

                while (copy_seq_len) {
                    encoder->encoded[encoder->pos_enco] =
                        encoder->decoded[encoder->pos_deco];
                    ++(encoder->pos_enco);
                    ++(encoder->pos_deco);
                    --copy_seq_len;
                }
